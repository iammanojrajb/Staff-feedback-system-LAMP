...
<?php  
    ...
    if(!mysqli_select_db($Conn,$depname))
        ...
        {   
        if($_POST['newname'] and $_POST['oldname'] )
        {
            ...
            if($equ)
            {
                $newname=str_replace("$"," ",$newname);
                $newname=str_replace("_",".",$newname);
                echo "<script>alert('$newname updated Successfully in $depname Department')</script>
                ";
            }
        }else
        {
            echo "<script>alert('Invalid option')</script>";
        }
        }
...
<center style="font-size:40px;font-family: 'Gafata';"><u style="color: #AC3B61">EDIT FACULTY</u>
    <form name="deletefacultyform" action="" method="post">
        <div class="heads1">
        <div><span>Choose Name:</span></div>
        </div>

        <div class="heads2"><div><select name="oldname">
       <?php
     ...
     <div class="heads">
     <div><span>Alter Name:</span></div>
     </div>
     <div class="login"><input type="text" name="newname"/><br>
