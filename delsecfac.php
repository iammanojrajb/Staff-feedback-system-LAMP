...
<?php
    ...
    if(isset($_POST["submit"]))
        {
            $facname=$_POST['delete'];
            //echo "<script>alert('$facname')</script>";
...
//echo "<script>alert(' $facname ')</script>";
             ...
             $qname="ALTER TABLE $t DROP $facname";
              if($qname)
            {
                //echo "work1";
                //echo "$sem";
  ...
  $facname=str_replace("%"," ",$facname);
               ...
               echo "<script>alert(' $facname Deleted Successfully')</script>";
            }
        }   
...
<div class="heads">Choose Faculty:</div>
<?php
$s=0;
        $s=0;
